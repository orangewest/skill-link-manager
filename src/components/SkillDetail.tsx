    ...
    <div>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-xl font-bold tracking-tight text-ink">{detail.name}</h2>
          <div className="flex flex-shrink-0 items-center gap-2">
            ...
            <Tooltip text={t("openFolder")}>
              <button
                ...
                className="rounded-card border border-hairline p-1.5 text-ink-2 transition-colors hover:bg-fill dark:border-hairline dark:text-ink-4 dark:hover:bg-hover"
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4">
            ...
            <Tooltip text={actionLoading === "all" ? t("applying") : allLinked ? t("unlinkAll") : t("linkAll")}>
              <button
                ...
                className={
                  allLinked
                    ? "rounded-card bg-red-50 p-1.5 text-red-600 transition-colors hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-red-900/30 dark:text-red-400 dark:hover:bg-red-900/50"
                    : "rounded-card bg-accent-bg p-1.5 text-accent transition-colors hover:bg-accent-bg disabled:cursor-not-allowed disabled:opacity-50 dark:bg-accent-bg dark:text-accent dark:hover:bg-accent-bg"
                }
              >
                {allLinked ? (
            ...
            <Tooltip text={t("deleteSkill")}>
              <button
                ...
                className="rounded-card border border-red-200 p-1.5 text-red-600 transition-colors hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-red-800 dark:text-red-400 dark:hover:bg-red-900/30"
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4">
        ...
        <p className="mt-2 whitespace-pre-wrap text-sm text-ink-2 dark:text-ink-4">
          {detail.description || "\u2014"}
        </p>
      </div>
      ...
      <div className="divide-y divide-hairline overflow-hidden rounded-card border border-hairline bg-surface shadow-card">
        {detail.linked_dirs.map((dir) => (
          <div
            key={dir.name}
            className="flex items-center justify-between p-3 transition-colors hover:bg-hover"
          >
            <div className="min-w-0 flex-1">
              <div className="font-medium text-ink dark:text-ink">{dir.name}</div>
              <div className="truncate text-xs text-ink-4 dark:text-ink-3">{dir.path}</div>
            </div>
            <div className="flex flex-shrink-0 items-center gap-2">
              {dir.linked ? (
                <>
                  ...
                  <Tooltip text={t("removeLink")}>
                    <button
                      ...
                      className="rounded bg-red-50 p-1.5 text-red-600 transition-colors hover:bg-red-100 disabled:bg-fill disabled:text-ink-4 dark:bg-red-900/30 dark:text-red-400 dark:hover:bg-red-900/50 dark:disabled:bg-fill dark:disabled:text-ink-3"
                    >
                      <UnlinkIcon />
                    </button>
                  </Tooltip>
                ...
                <>
                  ...
                  <Tooltip text={t("addLink")}>
                    <button
                      ...
                      className="rounded bg-accent-bg p-1.5 text-accent transition-colors hover:bg-accent-bg disabled:bg-fill disabled:text-ink-4 dark:bg-accent-bg dark:text-accent dark:hover:bg-accent-bg dark:disabled:bg-fill dark:disabled:text-ink-3"
                    >
                      <LinkIcon />
                    </button>
                  </Tooltip>
      ...
      {showDeleteStep1 && (
          ...
          footer={
            <>
              ...
              <button
                ...
                className="rounded-card bg-red-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-red-700"
              >
                {t("continueButton")}
              </button>
            </>
        ...
        >
          <p className="text-sm text-ink-2 dark:text-ink-4">{t("deleteSkillConfirmBody")}</p>
        </Modal>
      )}
      ...
      {showDeleteStep2 && detail && (
          ...
          footer={
            <>
              ...
              <button
                ...
                className="rounded-card bg-red-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-red-700 disabled:cursor-not-allowed disabled:bg-fill dark:disabled:bg-fill"
              >
        ...
        >
          <p className="text-sm text-ink-2 dark:text-ink-4">
            {t("deleteSkillFinalBody")} <span className="font-mono font-semibold text-ink dark:text-ink">{detail.name}</span>
          </p>
          <input
            type="text"
            value={confirmInput}
            ...
            autoFocus
            className="mt-3 w-full rounded-card border border-hairline bg-surface px-3 py-2 text-sm focus:border-red-400 focus:outline-none focus:ring-1 focus:ring-red-400 dark:border-hairline dark:bg-surface dark:text-ink"
          />
        </Modal>
      )}
    </div>
